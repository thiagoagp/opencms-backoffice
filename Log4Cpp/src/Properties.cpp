...
namespace log4cpp {
    ...
    int Properties::getInt(const std::string& property, int defaultValue, bool measured) {
        const_iterator key = find(property);
        int value = defaultValue;
        if(key != end()){
			std::string propValue = (*key).second;
			value = std::atoi(propValue.c_str());
			if(measured){
				char buffer[1024] = {"\0"};
				itoa(value, buffer, 10);
				std::string stringValue(buffer);

				if(propValue.length() != stringValue.length()){
					std::string measure = StringUtil::toLowerCase(propValue.substr(stringValue.length()));
