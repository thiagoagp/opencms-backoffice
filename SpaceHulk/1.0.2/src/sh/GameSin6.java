...
public class GameSin6 extends Game
{
    ...
    public final static String INTRO = "/maps/16.html";
        ...
        throws IOException
    {
        ...
        // TODO Flamers only have 4 shots
    }
        ...
        if (object == TileType.OBJECT_TOXIN && (p instanceof Blip || p instanceof Stealer))
        {
            if (p.useActionPoints(1))
            {
                ...
                if (c >= 6)
                {
                    ...
                    getListener().pieceCloseCombatMiss(p);
            }
            return true;
        }
        else
    ...
    {
        // Check flamer is not used in duct room or joining corridors
        boolean flame = false;
        for (int x = 15; !flame && x <= 17; ++x)
            flame = getMap().isFire(x, 23);
        for (int y = 24; !flame && y <= 28; ++y)
            flame = getMap().isFire(16, y);
        for (int y = 29; !flame && y <= 31; ++y)
            for (int x = 15; !flame && x <= 17; ++x)
                flame = getMap().isFire(x, y);

        if (flame)
            return true;
        else if (getMap().getObject(16, 31) != TileType.OBJECT_TOXIN)
            return true;
        else
            return marines_.size() <= 0;
    }
}
