...
public final class URL {
    ...
    public URL(String spec) throws MalformedURLException {
        int start = 0;
        int index = spec.indexOf(":", start);
        if(index < 0) {
            throw new MalformedURLException("The string is not a valid url.");
        }
        protocol = spec.substring(0, index);
        if(!isValidProtocol(protocol)) {
            throw new MalformedURLException("The string hasn't a valid prtocol.");
        }
        ...
        if(index < 0)
            index = spec.length();
        host = spec.substring(start, index);
        start = index + 1;
        ...
        index = spec.lastIndexOf('#');
        if(index > 0) {
            ref = spec.substring(index + 1, end);
            end = index;
        }
        else {
            end = spec.length();
    ...
    private boolean isValidProtocol(String protocol) {
        ...
        if (!com.mscg.util.lang.Character.isLetter(c)) {
            return false;
        }
        for (int i = 1; i < len; i++) {
            c = protocol.charAt(i);
            if (!com.mscg.util.lang.Character.isLetterOrDigit(c) && c != '.' && c != '+' &&
                    c != '-') {
                return false;
            }
        }
