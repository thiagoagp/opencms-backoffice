 ...
 * its entries were last accessed, from least-recently accessed to most-recently
 * (<i>access-order</i>). This kind of map is well-suited to building LRU
 * caches. Invoking the <tt>put</tt> or <tt>get</tt> method results in an access
 * to the corresponding entry (assuming it exists after the invocation
 * completes). The <tt>putAll</tt> method generates one entry access for each
 ...
 * for the basic operations (<tt>add</tt>, <tt>contains</tt> and <tt>remove</tt>
 * ), assuming the hash function disperses elements properly among the buckets.
 * Performance is likely to be just slightly below that of <tt>HashMap</tt>, due
 * to the added expense of maintaining the linked list, with one exception:
 * Iteration over the collection-views of a <tt>LinkedHashMap</tt> requires time
