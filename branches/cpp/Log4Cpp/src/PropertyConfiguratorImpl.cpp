...
namespace log4cpp {
	...
	const std::string PropertyConfiguratorImpl::rootCategoryName   = "rootLogger";
    const std::string PropertyConfiguratorImpl::categoryNamePrefix = "logger";
    ...
    void PropertyConfiguratorImpl::configureCategory(const std::string& categoryName) throw (ConfigureFailure) {
        ...
        std::string tempCatName = (categoryName == PropertyConfiguratorImpl::rootCategoryName) ?
            categoryName :
            PropertyConfiguratorImpl::categoryNamePrefix + "." + categoryName;
        ...
        Category& category = (categoryName == PropertyConfiguratorImpl::rootCategoryName) ?
    ...
    Appender* PropertyConfiguratorImpl::instantiateAppender(const std::string& appenderName) {
        ...
        if (appenderType == "ConsoleAppender") {
        ...
        else if(appenderType == "StringQueueAppender" || appenderType == "StringListAppender") {
        	int maxLines = _properties.getInt(appenderPrefix + ".MaxLines", 0);
        	if(appenderType == "StringQueueAppender"){
        		appender = new StringQueueAppender(appenderName, maxLines);
        	}
        	else /*if(appenderType == "StringListAppender")*/{
        		appender = new StringListAppender(appenderName, maxLines);
        	}
        }
        else if(appenderType == "NotifierAppender") {
        	bool deleteListenersOnExit = _properties.getBool(appenderPrefix + ".DeleteOnExit", false);
        	appender = new NotifierAppender(appenderName, deleteListenersOnExit);
        }
        else if (appenderType == "FileAppender") {
            std::string fileName = _properties.getString(appenderPrefix + ".File", "foobar");
            ...
            std::string fileName = _properties.getString(appenderPrefix + ".File", "foobar");
            size_t maxFileSize = _properties.getInt(appenderPrefix + ".MaxFileSize", 10*1024*1024, true);
    ...
    void PropertyConfiguratorImpl::getCategories(std::vector<std::string>& categories) const {
        ...
        categories.push_back(std::string(PropertyConfiguratorImpl::rootCategoryName));

        // then look for "category."
        std::string prefix(PropertyConfiguratorImpl::categoryNamePrefix);
        Properties::const_iterator from = _properties.lower_bound(prefix + '.');
