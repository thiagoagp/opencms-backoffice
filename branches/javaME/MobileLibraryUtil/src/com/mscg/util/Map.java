 ...
 * 
 * <p>
 * This interface takes the place of the <tt>Dictionary</tt> class, which was a
 ...
 * in a manner that affects <tt>equals</tt> comparisons while the object is a
 * key in the map. A special case of this prohibition is that it is not
 * permissible for a map to contain itself as a key. While it is permissible for
 * a map to contain itself as a value, extreme caution is advised: the
 * <tt>equals</tt> and <tt>hashCode</tt> methods are no longer well defined on
 * such a map.
 * 
 * <p>
 ...
 * and a constructor with a single argument of type <tt>Map</tt>, which creates
 * a new map with the same key-value mappings as its argument. In effect, the
 * latter constructor allows the user to copy any map, producing an equivalent
 * map of the desired class. There is no way to enforce this recommendation (as
 ...
 * throw an <tt>UnsupportedOperationException</tt> if the invocation would have
 * no effect on the map. For example, invoking the {@link #putAll(Map)} method
 ...
 * the insertion of an ineligible element into the map may throw an exception or
 * it may succeed, at the option of the implementation. Such exceptions are
 ...
 * for the {@link #containsKey(Object) containsKey(Object key)} method says:
 * "returns <tt>true</tt> if and only if this map contains a mapping for a key
 * <tt>k</tt> such that <tt>(key==null ? k==null : key.equals(k))</tt>." This
 * specification should <i>not</i> be construed to imply that invoking
 * <tt>Map.containsKey</tt> with a non-null argument <tt>key</tt> will cause
 * <tt>key.equals(k)</tt> to be invoked for any key <tt>k</tt>. Implementations
 * are free to implement optimizations whereby the <tt>equals</tt> invocation is
 * avoided, for example, by first comparing the hash codes of the two keys. (The
...
public interface Map {
	...
	interface Entry {
		 ...
		 * @throws IllegalArgumentException
		 *             if some property of this value prevents it from being
