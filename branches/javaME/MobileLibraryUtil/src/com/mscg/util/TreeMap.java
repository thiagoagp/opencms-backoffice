...
public class TreeMap extends AbstractMap implements NavigableMap {
	 ...
	 */
	final Entry getCeilingEntry(Object key) {
		Entry p = root;
		while (p != null) {
	 ...
	 */
	final Entry getFloorEntry(Object key) {
		Entry p = root;
		while (p != null) {
			int cmp = compare(key, p.key);
			if (cmp > 0) {
	 ...
	 */
	final Entry getHigherEntry(Object key) {
		Entry p = root;
		while (p != null) {
			int cmp = compare(key, p.key);
			if (cmp < 0) {
	 ...
	 */
	final Entry getLowerEntry(Object key) {
		Entry p = root;
		while (p != null) {
			int cmp = compare(key, p.key);
			if (cmp > 0) {
	...
	static final class KeySet extends AbstractSet implements NavigableSet {
		...
		public Iterator descendingIterator() {
			if (m instanceof TreeMap)
				...
				return (Iterator) (((TreeMap.NavigableSubMap) m)
	 ...
	 */
	static Object keyOrNull(TreeMap.Entry e) {
	 ...
	 * @serial include
	 */
	static abstract class NavigableSubMap extends AbstractMap implements NavigableMap {
		/**
		 * The backing map.
		 */
		final TreeMap m;
		...
		NavigableSubMap(TreeMap m, boolean fromStart, Object lo,
				boolean loInclusive, boolean toEnd, Object hi,
				boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		final TreeMap.Entry absLowest() {
			TreeMap.Entry e = (fromStart ? m.getFirstEntry() : (loInclusive ? m
					.getCeilingEntry(lo) : m.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final TreeMap.Entry absHighest() {
			TreeMap.Entry e = (toEnd ? m.getLastEntry() : (hiInclusive ? m
					.getFloorEntry(hi) : m.getLowerEntry(hi)));
		...
		abstract TreeMap.Entry subCeiling(Object key);

		abstract TreeMap.Entry subHigher(Object key);

		abstract TreeMap.Entry subFloor(Object key);

		abstract TreeMap.Entry subLower(Object key);
		...
		public boolean isEmpty() {
			return (fromStart && toEnd) ? m.isEmpty() : this.entrySet()
					.isEmpty();
		}

		public int size() {
			return (fromStart && toEnd) ? m.size() : this.entrySet().size();
		}
		...
		abstract class SubMapIterator implements Iterator {
			...
			SubMapIterator(TreeMap.Entry first, TreeMap.Entry fence) {
		...
		final class SubMapEntryIterator extends SubMapIterator {
			SubMapEntryIterator(TreeMap.Entry first, TreeMap.Entry fence) {
				super(first, fence);
			}
		...
		final class SubMapKeyIterator extends SubMapIterator {
			SubMapKeyIterator(TreeMap.Entry first, TreeMap.Entry fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapKeyIterator extends SubMapIterator {
			DescendingSubMapKeyIterator(TreeMap.Entry last, TreeMap.Entry fence) {
				super(last, fence);
			}
	...
	static final class AscendingSubMap extends NavigableSubMap {
		...
		AscendingSubMap(TreeMap m, boolean fromStart, Object lo,
				boolean loInclusive, boolean toEnd, Object hi,
				boolean hiInclusive) {
		...
		TreeMap.Entry subCeiling(Object key) {
			return absCeiling(key);
		}

		TreeMap.Entry subHigher(Object key) {
			return absHigher(key);
		}

		TreeMap.Entry subFloor(Object key) {
			return absFloor(key);
		}

		TreeMap.Entry subLower(Object key) {
			return absLower(key);
		}
	}
	...
	static final class DescendingSubMap extends NavigableSubMap {
		...
		DescendingSubMap(TreeMap m, boolean fromStart, Object lo,
				boolean loInclusive, boolean toEnd, Object hi,
				boolean hiInclusive) {
			super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
		}

		private final Comparator reverseComparator = new ReverseComparator(
				m.comparator);
	 ...
	 */
	private void buildFromSorted(int size, Iterator it, InputStream str,
			Object defaultVal) throws java.io.IOException,
	...
	private final Entry buildFromSorted(int level, int lo, int hi,
			int redLevel, Iterator it, InputStream str, Object defaultVal)
