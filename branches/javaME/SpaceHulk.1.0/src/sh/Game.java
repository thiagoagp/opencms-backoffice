...
public abstract class Game {
	...
	public Game(String map) throws IOException {
		map_ = new Map(map);
	...
	private void closeCombat(Piece a, Piece d, int dir) {
		util.Debug.assert2(!(a instanceof Blip),
				"Game::closeCombat blips cannot attack");
		util.Debug.assert2(!(d instanceof Blip),
				"Game::closeCombat blips cannot be attacked");
		util.Debug.assert2(dir == Direction.getDirection(a.getPosX(), a
				.getPosY(), d.getPosX(), d.getPosY()),
				"Game::closeCombat dir is invalid");
		util.Debug.assert2(Face.isForward(dir, a.getFace()),
				"Game::closeCombat Defender must be in front of Attacker");

		int odir = Direction.flip(dir);
		util.Debug.assert2(odir == Direction.getDirection(d.getPosX(), d
				.getPosY(), a.getPosX(), a.getPosY()),
				"Game::closeCombat odir is invalid "
						+ odir
						+ " "
						+ Direction.getDirection(d.getPosX(), d.getPosY(), a
								.getPosX(), a.getPosY()));
	...
	private void createBlip(int count) {
		...
		while (count > 0) {
			...
			if (blip == null)
				break;
			// util.Debug.message(" Game::createBlip placing blip " +
			// blip.getPosX() + " " + blip.getPosY());
			map_.placePiece(blip);
			blips_.addElement(blip);
			--count;
			util.Debug.assert2(
					map_.getPiece(blip.getPosX(), blip.getPosY()) == blip,
					"Blip not placed");
		}
		// util.Debug.message("-Game::createBlip");
	}
	...
	private void createMarine(Map.MarineStart ms, Map.StartPos sp) {
		// util.Debug.message("createMarine: " + ms.name + " " + sp.x + "," +
		// sp.y);
...
//									m.setOverwatch(false);
	...
	public void drop(Marine m) {
		...
		if (object != 0 && map_.getItem(m.getPosX(), m.getPosY()) == 0
				&& !m.getOverwatch()) {
	...
	Vector findMarineLineofSight(int x, int y, boolean fireArc) {
		for (int i = 0; i < marines_.size(); ++i) {
			// util.Debug.message("+Game::findMarineLineofSight " + x + " " +
			// y);
			Marine m = (Marine) marines_.elementAt(i);
			...
			if (path != null && path.size() > 1) {
				...
				int dir = Direction.getDirection(m.getPosX(), m.getPosY(), p.x,
						p.y);
				util.Debug.assert2(dir != Direction.NONE, "Invalid direction");
		...
		for (int i = 0; i < blips_.size(); ++i) {
			...
			if (!blip.isDeleted()) {
				...
				for (int di = 0; di < Direction.ordered8.length; ++di) {
					...
					Piece p = map_.getPiece(blip.getPosX()
							+ Direction.getOffsetX(d), blip.getPosY()
							+ Direction.getOffsetY(d));
				...
				if (!flipped) {
					Vector path = findMarineLineofSight(blip.getPosX(), blip
							.getPosY(), false);
					if (path != null) {
						// util.Debug.message("Game::flipBlips found");
						Point2I p = (Point2I) path.elementAt(1);
						int dir = Direction.getDirection(blip.getPosX(), blip
								.getPosY(), p.x, p.y);
	...
	public int getStealearKills() {
		return stealerKills_;
	}
	...
	boolean move(Blip blip, int dir) {
		...
		if (DoorState.getState(TileType.DOOR, object) == DoorState.CLOSED) {
			...
			case Direction.NORTH_WEST:
				canmove = canmove
						&& (map_.canMove(x - Direction.getOffsetX(dir), y,
								true, true, true) || map_.canMove(x, y
								- Direction.getOffsetY(dir), true, true, true));
				break;
			}
			if (canmove) {
				if (blip.useActionPoints(1)) {
					...
					moved = true;
					if (findMarineLineofSight(blip.getPosX(), blip.getPosY(),
							false) != null)
						flipBlip(blip, dir);
				}
			}
	...
	public void move(Marine m, int dir, boolean preferTurn) {
		...
		// util.Debug.message("Game::move marine " + m.getPosX() + ", " +
		// m.getPosY() + "  " + x + ", " + y);
			...
			if (p instanceof Stealer) {
				...
				if (m.useActionPoints(1)) {
					...
					if (c >= 6) {
						map_.blastDoor(x, y, gl_);
						flipBlips();
						gl_.pieceCloseCombatObject(m, x, y, object);
					} else
						gl_.pieceCloseCombatMiss(m);
				}
				...
				case Direction.NORTH_WEST:
					canmove = canmove
							&& (map_.canMove(x - Direction.getOffsetX(dir), y,
									true, true, true) || map_.canMove(x, y
									- Direction.getOffsetY(dir), true, true,
									true));
					break;
				}
				if (canmove) {
	...
	boolean move(Stealer s, int dir, boolean preferTurn) {
		...
		// util.Debug.message("Game::move stealer " + s.getPosX() + ", " +
		// s.getPosY() + "  " + x + ", " + y);
			...
			while (i < startPos.size()) {
				...
				if (sp.name != null) {
					for (int j = 0; j < marineStart.size(); ++j) {
						Map.MarineStart ms = (Map.MarineStart) marineStart
								.elementAt(j);
					...
					if (!found)
						util.Debug.error("Game::placeMarines: cannot find "
								+ sp.name);
				}
				if (found)
	...
	public void shoot(Marine m, int x, int y) {
		if (!m.getJammed() && !m.getOverwatch()) {
			if (m.getType() == Marine.FLAMER) {
				...
				if (Face.isForward(dir, m.getFace()) && m.getAmmunition() >= 1
						&& m.canUseActionPoints(2)) {
					...
					IFovBoard b = new FlamerIFovBoard(this, m);

					// util.Debug.message("Game::shoot Flamer m " + m.getPosX()
					// + " " + m.getPosY());
					// util.Debug.message("Game::shoot Flamer s " + x + " " +
					// x);
				...
				if (p instanceof Stealer) {
					if (m.useActionPoints(1)) {
						...
						gl_.pieceShoots(m);
						m.setTarget(p);
						int shotValue = m.getShootValue(r_, false, gl_);
						if (shotValue >= 6) {
							// util.Debug.message("Marine shoots Stealer");
							kill(p);
						} else
	...
	public boolean toggleDoor(Marine m, int dir) {
		...
		int x = m.getPosX() + Direction.getOffsetX(dir);
		int y = m.getPosY() + Direction.getOffsetY(dir);
		if (!m.getOverwatch() && Face.isForward(dir, m.getFace())
			...
			if (tileType == TileType.DOOR || tileType == TileType.BULKHEAD) {
				...
				if (ds == DoorState.OPEN
						|| (tileType != TileType.BULKHEAD && ds == DoorState.CLOSED)) {
					if (m.useActionPoints(1)) {
						changed = true;
