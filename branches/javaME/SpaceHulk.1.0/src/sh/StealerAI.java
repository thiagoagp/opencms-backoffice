...
// TODO Need to implement a point system, dont just go after nearest target
...
class StealerAI
{
    ...
    private class StealerPathBoard implements IPathBoard
    {
        ...
        public boolean isObstacle(int x, int y)
        {
            ...
            {
                boolean surroundingCell = Math.abs(x - p_.getPosX()) <= 1
                    && Math.abs(y - p_.getPosY()) <= 1;
        ...
        public boolean isEnd(int x, int y)
        {
            if (targetObject_ != 0)
            {
                if (targetObject_ == map_.getObject(x, y))
                    return true;
                else
                {
            ...
            else
            {
                ...
                {
                    Marine m = (Marine) p;
                    if (marineTargetType_ != Marine.STANDARD)
                        return marineTargetType_ == m.getType();
                    else
                        return true;
    ...
    private void moveBlips()
    {
        ...
        //util.Debug.message("moveBlips " + blips.size());
        for (int i = 0; i < blips.size(); ++i)
        {
            ...
            if (!blip.isDeleted())
            {
                ...
                if (as.findPath(blip.getPosX(), blip.getPosY(), new StealerPathBoard(blip)))
                {
                    ...
                    // Flip if we can reach the Marine
                    boolean flip = path.size() <= (blip.getActionPoints() + 1);
                    if (!flip)
                    {
                        // Or we will move into a Marines LOS
                    ...
                    if (flip)
                    {
                        //util.Debug.message("Game::moveBlips flip");
    ...
    private void moveStealer(Stealer stealer)
    {
        ...
        if (as.findPath(stealer.getPosX(), stealer.getPosY(), new StealerPathBoard(stealer)))
        {
            ...
            for (int j = 0; j < path.size(); ++j)
            {
                ...
                if (!game_.move(stealer, dir, (j + 1) < path.size()))
                    break;
    ...
    private void moveStealers()
    {
        ...
        //util.Debug.message("moveStealers " + stealers.size());
