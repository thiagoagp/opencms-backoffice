...
public class SpaceHulk extends MIDlet {
    ...
    private class InitMenuCallback implements Callback {
        ...
        public void perform() {
            ...
            mv.add("Dice analyzer", new Callback() {

				public void perform() {
					showDiceAnalyzer(InitMenuCallback.this);
				}
...
//        }
    ...
    private void showDiceAnalyzer(Callback back) {
    	int count[] = new int[6];
    	int lastRoll = 0;
    	int sequences = 0;
    	for(int i = 0; i < 600000; i++) {
    		int roll = Dice6.getDice().getDiceRoll();
    		if(roll == lastRoll)
    			sequences++;
    		count[roll - 1]++;
    		lastRoll = roll;
    	...
    	VerticalView vv = new VerticalView();
        View titleView = createTitle("Dice analys results");
        vv.add(titleView);
        vv.add(new SpaceView(20, 20));
        for(int i = 0; i < 6; i++) {
    		vv.add(new StringView("" + (i + 1) + ": " + count[i]));
    	}
        vv.add(new SpaceView(20, 20));
        vv.add(new StringView("Sequences: " + sequences));
