...
public class DocumentScanner {
	...
	//private static final Log LOG = LogFactory.getLog(DocumentScanner.class);
	...
	/**
      ...
      * policy is in effect.
      */
	public void setLiberalPolicyAreaWhitespaceFraction
		(float liberalPolicyAreaWhitespaceFraction) {
	...
	/**
      ...
      */
	public void setMinCharBreakWidthAsFractionOfRowHeight
		(float minCharBreakWidthAsFractionOfRowHeight) {
	...
	/**
      ...
      * will be drawn to denote the borders of rows and character cells.  This
      * is used for debugging and can be <code>null</code>.
      */
...
///LOG.debug("left blackCount="+blackCount+" maxBlack="+maxBlack+" blockY1="+blockY1+" blockY2="+blockY2);
	    if (blackCount >= maxBlack) {
///LOG.debug("    reduce left");
		reduced = true;
		blockX1++;
	    ...
	    for (int y = blockY1; y <= blockY2; y++, idx += w) {
///LOG.debug("["+pixels[idx]+"]");
		if (pixels[idx] < thresh) blackCount++;
	    }
///LOG.debug();
///LOG.debug("right blackCount="+blackCount+" maxBlack="+maxBlack+" blockY1="+blockY1+" blockY2="+blockY2);
	    if (blackCount >= maxBlack) {
///LOG.debug("    reduce right");
		reduced = true;
		blockX2--;
		if (blockX1 >= blockX2) break;
	    }
...
///LOG.debug("top blackCount="+blackCount+" maxBlack="+maxBlack+" blockX1="+blockX1+" blockX2="+blockX2);
	    if (blackCount >= maxBlack) {
///LOG.debug("    reduce top");
		reduced = true;
		blockY1++;
...
///LOG.debug("bottom blackCount="+blackCount+" maxBlack="+maxBlack+" blockX1="+blockX1+" blockX2="+blockX2);
	    if (blackCount >= maxBlack) {
///LOG.debug("    reduce bottom");
		reduced = true;
		blockY2--;
		if (blockY1 >= blockY2) break;
	    }
...
*/
		...
		for (int i = 0; (i + 1) < al.size(); i += 2) {
			...
			if (g != null) {
				drawBox(g, blockX1 - 1, bY1 - 1, blockX2, bY2,
						Color.green);
			}
///
///LOG.error("process row: "+blockX1+","+bY1+" "+blockX2+","+bY2);
			processRow(pixelImage,
					   listener,
					   pixels, w, h, blockX1, bY1, blockX2, bY2, g);
		}
	}
		...
		for (int i = 0; (i + 1) < al.size(); i += 2) {
			if (i >= 2) {
				...
				while ((cx2 - cx1) >= minSpaceWidth) {
					...
					if (g != null) {
						drawBox(g, cx1 - 1, y1 - 1, sx2, y2, Color.blue);
					}
			...
			if (cy1 >= cy2) {
				...
				if (g != null) {
					drawBox(g, cx1 - 1, y1 - 1, cx2, y2, Color.blue);
				}
				listener.processSpace(pixelImage, cx1, y1, cx2, y2);
			} else {
				if (g != null) {
					drawBox(g, cx1 - 1, cy1 - 1, cx2, cy2, Color.red);
				}
	...
	private final void drawBox(Graphics g, int x1, int y1, int x2, int y2, Color color) {
